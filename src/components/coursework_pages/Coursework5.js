      ...
      return (
         <div className="coursework5">
            ...
            <div className="coursework5-right">
               {this.state.c1 ? (<p className="course-name">Computer Vision</p>) : (<div/>)}
               {this.state.c1 ? (<p className="course-desc">An introduction to basic concepts and techniques in computervision. This includes low-level operations such as image filtering and edge detection, 3D reconstruction of scenes using stereo and structure from motion, and object detection, recognition and classification.</p>) : (<div/>)}
               {this.state.c2 ? (<p className="course-name">Cybersecurity Group Problem Solving</p>) : (<div/>)}
               {this.state.c2 ? (<p className="course-desc">Students team up to solve a cybersecurity-related project throughout the semester-long course.</p>) : (<div/>)}
               {this.state.c3 ? (<p className="course-name">IoT Security</p>) : (<div/>)}
               {this.state.c3 ? (<p className="course-desc">This course will expose students to topics in Internet of Things (IoT) and Cyber Physical System (CPS) device types, IoT/CPS threat categories, security services, distributed networking, activity privacy, and intrusion detection for embedded environments. In addition to individual homework assignments, students will participate in a semester long group project involving research, design, and implementation.</p>) : (<div/>)}
               {this.state.c4 ? (<p className="course-name">Data Science</p>) : (<div/>)}
               {this.state.c4 ? (<p className="course-desc">An introduction to the data science pipeline, i.e., the end-to-end process of going from unstructured, messy data to knowledge and actionable insights. Provides a broad overview of several topics including statistical data analysis, basic data mining and machine learning algorithms, large-scale data management, cloud computing, and information visualization.</p>) : (<div/>)}
               {this.state.c5 ? (<p className="course-name">French Literature and Composition</p>) : (<div/>)}
               {this.state.c5 ? (<p className="course-desc">Introduction to cultural and textual analysis of selected readings from various genres in French literature. Taught in French.</p>) : (<div/>)}
            </div>
         </div>
       );
   }
